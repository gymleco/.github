...
데스크톱 (≥1024px)
┌──────────────────────────────────────────────────────────────────┐
│ GYMLECO   제품 중고 부품 악세사리 공식헬스장 브랜드   고객센터▾ ☀ 문의│
└──────────────────────────────────────────────────────────────────┘
                                                    └ 문의하기 / FAQ / 공지사항

모바일
┌──────────────────────────────────────────────────────────────────┐
│ GYMLECO        제품  중고  부품  …  (가로 스크롤)          ☀  문의 │
└──────────────────────────────────────────────────────────────────┘
```

- 메뉴가 넘치면 **가로로 스크롤**합니다. 햄버거를 쓰지 않습니다 —
  항목이 잘려 보이는 것 자체가 «더 있다» 는 신호입니다.
  OS 스크롤바는 `.no-scrollbar` 로 감춥니다(헤더 한가운데 굵은 띠가 생김).
- 홈에서도 **같은 헤더**를 씁니다. 예전에는 홈만 자체 헤더(3개 메뉴)라
  홈에서 시작한 사람은 중고·부품·공식 헬스장으로 가는 길이 아예 없었습니다.
- **테마 토글**은 CSS 로 아이콘을 고릅니다(`.theme-icon-light/dark`).
  JS 상태로 고르면 수화 전까지 반대쪽 아이콘이 잠깐 보입니다.
...
- **언제 연락이 가는지 반드시 적습니다.** 기다림이 불안하면 다른 업체에도
  문의하게 됩니다.
- 대체 경로(전화)를 같이 둡니다. ⚠️ 지금 번호는 자리표시자입니다.
...
| `/support/*` 제목 | **버그** | 「자주 묻는 질문 \| GYMLECO KOREA \| GYMLECO KOREA」 — 사이트명 중복 |
| 대표 전화번호 | **`02-0000-0000`** | 접수 완료 화면·푸터의 자리표시자 |
