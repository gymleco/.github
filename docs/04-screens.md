...
**기준: 2026-09-09 로컬 실행분 (`localhost:3111`).**

> 실제 화면 캡처와 페이지 HTML 은 `기획서/캡처/`, `기획서/화면html/` 에 있습니다.
> 사람이 보는 판은 `기획서/08_화면설계도_PC.html`, `09_화면설계도_MO.html` 입니다.
...
공개 라우트 **16개**, Route Handler 3개(`/api/inquiries`, `/api/revalidate`, `/api/verify`).
...
| 메뉴 | 한 줄 + 가로 스크롤 | **두 줄 헤더(가운데 정렬) + 모바일 서랍 아코디언** |
...
| **고객센터** | `/contact` | 견적 문의 · 무료 시연 신청 · **정품 확인** · 자주 묻는 질문 · 공지사항 · 개인정보처리방침 |
...
├──────────────────────────────────────────────────────────────┤
│         제품 라인업   중고 기구   부품   악세사리             │
└──────────────────────────────────────────────────────────────┘
  윗줄 = 묶음(가운데 정렬) · 아랫줄 = 지금 묶음의 하위(늘 펼쳐 둠)
  지금 보고 있는 항목은 브랜드색으로 표시
...
**드롭다운을 걷어냈습니다.** 드롭다운은 «누르기 전까지 보이지 않는» 메뉴입니다.
마우스를 정확히 얹고 있어야 열려 있고, 살짝 벗어나면 닫힙니다. 하위가 다섯인
고객센터에서 특히 불편했습니다. 대신 아랫줄에 **펼쳐 둡니다** — 지금 보고 있는
곳의 형제 항목이 늘 보이므로, 「제품 라인업 옆에 중고가 있다」를 열어 보지 않아도
압니다.

**윗줄은 가운데 정렬입니다.** `flex` + `justify-between` 으로 두면 로고와 오른쪽
버튼의 폭이 달라 메뉴가 «화면 가운데» 가 아니라 «남은 자리의 가운데» 에 놓입니다.
양쪽을 `1fr` 로 잡은 세 칸 격자여야 정가운데에 섭니다.

**누르는 자리를 넓혔습니다** (`px-5 py-3`). 글자만큼만 잡아 두면 항목 사이 빈틈을
눌러 아무 일도 안 일어나는 일이 생깁니다.

### 아랫줄이 언제 «자리를 차지하는가»

| 지금 페이지 | 아랫줄 | 다른 묶음에 마우스를 얹으면 |
|---|---|---|
| 묶음에 속함 (`/products`, `/support/faq` …) | **늘 있음** | 그 자리에서 내용만 바뀜 — 높이 안 흔들림 |
| 어느 묶음에도 없음 (`/`, `/contact/done`) | 없음 | **떠서** 보임 (`absolute`) |

두 번째 줄이 중요합니다. 홈에서 마우스를 얹었다 뗄 때마다 아랫줄이 생겼다
사라지면 **본문이 40px 씩 오르내립니다.** 그때만 띄우면 보이는 모습은 같고
본문은 움직이지 않습니다.
...
**헤더가 클라이언트 컴포넌트가 됐습니다.** 「지금 어느 묶음에 있나」와
「어디에 마우스를 얹었나」를 둘 다 알아야 아랫줄을 그릴 수 있습니다. 다만
`usePathname` 은 서버 렌더에서도 값을 주므로, **첫 HTML 에 이미 아랫줄이
들어 있습니다** — JS 가 늦어도 메뉴가 비지 않습니다. 마우스로 바꿔 보는 것만
JS 가 얹습니다. 묶음 이름 자체가 링크라 그마저 안 되어도 길이 막히지 않습니다.
...
│ │ 짐레코가 발급한 번호입니다                 │           │
│ │ 기구 · 디와이 로우 모델 017                │           │
│ │ 일련번호 · 017-2024-0117   제조 · 2024년   │           │
│ │ ▎위 기구가 눈앞의 기구와 같은지 확인해 주세요 │        │
│ └────────────────────────────────────────────┘           │
│ 알아두실 것 — 명판 위치 · 표기 규칙 · 중고 구매 시       │
└──────────────────────────────────────────────────────────┘
```

이미테이션 대응의 공개 창구입니다. 결과는 넷입니다.

| 결과 | 화면이 말하는 것 |
|---|---|
| `GENUINE` | 「짐레코가 발급한 번호입니다」 + 기구·연식 |
| `MODEL_MISMATCH` | 「번호와 모델이 맞지 않습니다」 — **다른 정보 일절 없음** |
| `UNKNOWN` | 「등록되지 않은 번호입니다」 |
| `NEEDS_CHECK` | 「확인이 필요한 기구입니다」 — 「도난」이라 말하지 않음 |

**「번호가 있다」로 끝내지 않습니다.** 위조범은 진짜 명판을 베껴 같은 번호를
여러 대에 붙입니다. 존재 여부만 답하면 우리가 가짜를 인증해 주는 꼴이 됩니다.
그래서 «이 번호의 기구는 무엇인가» 를 돌려주고, 눈앞의 기구와 대조하도록
화면이 대놓고 시킵니다.

**`MODEL_MISMATCH` 에는 아무것도 싣지 않습니다.** 「이 번호의 진짜 기구는
무엇이다」 를 알려 주면 위조범에게 다음에 어떤 기구에 붙여야 하는지 가르쳐
주는 셈입니다.

**모델번호를 필수로 받습니다.** 명판에 둘 다 적혀 있으니 소유자는 불편하지
않고, 번호를 순번으로 훑는 쪽은 (일련번호, 모델) 쌍을 알아야 하므로 훨씬
어려워집니다. IP 당 시간당 60회로 막고, **실패한 조회도 이력에 남깁니다** —
없는 번호 조회가 한 곳에서 몰리는 것이 번호 긁어가기입니다.
...
| 정품 확인 | **API** (`/api/public/verify`) | 실데이터 · 조회 이력 남김 |
...
| 일련번호 일괄 등록 · QR 라벨 | 없음 | 대장은 있으나 500대를 손으로 넣을 수는 없음 |
| 관리자 일련번호 화면 | **눈으로 미확인** | 코드·필드 대조는 통과. 로그인 뒤 렌더는 아직 못 봄 |
| 소식 API | 없음 | 테이블만 있음 |
