...
- [ ] **암호화 키도 함께 백업·복구 대상에 포함** ★
      `PII_ENCRYPTION_KEY` 를 잃으면 DB 를 복구해도 전화번호를 영영
      복호화할 수 없다. 암호문만 남고 복구 방법이 없다.
      `PII_BLIND_INDEX_KEY` 가 다르면 전화번호 검색이 전부 어긋난다.
      → 복구 훈련은 "DB + 키" 를 함께 복원해 **실제로 연락처가 읽히는지**
        확인하는 것까지여야 한다. DB 만 복구하고 끝내면 훈련이 아니다.
